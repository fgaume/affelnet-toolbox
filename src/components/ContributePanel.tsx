import { useState, useCallback, useRef, type ClipboardEvent, type DragEvent, type ChangeEvent } from 'react';
...
export function ContributePanel() {
    ...
    for (const item of items) {
      if (item.type.startsWith('image/')) {
        ...
        if (file) {
          ...
          const named = new File([file], `capture.${ext}`, { type: file.type });
          doUpload(named);
        }
        return;
      }
  ...
  return (
    <div className="contribute-panel">
      <h2 className="contribute-title">Contribuer — Envoyer une fiche-barème</h2>
      <p className="contribute-description">
        Envoyez votre fiche-barème (PDF, photo ou texte) pour enrichir les données
        de l'application. Ces fiches permettent de reconstituer les statistiques
        académiques et les seuils d'admission.
      </p>
      ...
      {uploadState.status === 'success' && (
        <div className="contribute-status contribute-status--success">
          ...
          Fichier envoyé : {uploadState.result?.original_filename} ({((uploadState.result?.size ?? 0) / 1024).toFixed(0)} Ko)
          <button className="contribute-reset" onClick={resetState}>Envoyer un autre</button>
        </div>
      )}
      {uploadState.status === 'error' && (
        <div className="contribute-status contribute-status--error">
          ...
          {uploadState.error}
          <button className="contribute-reset" onClick={resetState}>Réessayer</button>
        </div>
      )}
        ...
        <div className="contribute-sections">
          ...
          <section className="contribute-section">
            ...
            >
              ...
              <span className="contribute-dropzone-hint">PDF, PNG, JPG, WEBP — max 10 Mo</span>
              <input
                ref={fileInputRef}
                type="file"
          ...
          {/* Section 2: Paste image */}
          <section className="contribute-section">
            <h3>
              <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
                <path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z" />
              </svg>
              Coller une image (screenshot)
            </h3>
            <div
            ...
            >
              ...
              <span className="contribute-dropzone-hint">Screenshot Mac/Windows, copie depuis Aperçu, iPhone…</span>
            </div>
          </section>
          ...
          <section className="contribute-section">
            ...
            <textarea
              ...
              placeholder="Collez ici le contenu texte de votre fiche-barème…"
              value={pasteText}
